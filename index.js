...
function bilgisayarinSecimi() {
  ...
  const bilgisayarinSecimi = Math.random()

  if(bilgisayarinSecimi < 0.25) {
    console.log('Taş');
  }
  else if(bilgisayarinSecimi > 0.60) {
    console.log('Makas');
  }
  else {
    console.log('Kağıt');
...
function feetDonusturucu(Santimetre) {
  let feet;
  const feetDegeri = 30.48;

  feet = Santimetre / feetDegeri;
...
function cocukSarkisi(play) {
  for(let i = play; i >= 1; i--) {
